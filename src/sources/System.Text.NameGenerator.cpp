#include "System.Text.NameGenerator.hpp"

namespace System {
namespace Text {

NameGenerator::NameGenerator(): names({
...
NameGenerator::NameGenerator(const std::initializer_list<std::string>& descriptors, const std::initializer_list<std::string>& names) : names(names), desc(descriptors), generator() {}

std::string NameGenerator::Next() {
    std::uniform_int_distribution<int> distribution(0,names.size() - 1);
    int nameIdx = distribution(generator);
    
    distribution = std::uniform_int_distribution<int>(0, desc.size() - 1);
    int descIdx = distribution(generator);
    
    std::string desc = (this->desc)[descIdx];
    desc += "_";
    desc += (this->names)[nameIdx];
    return desc;
}
